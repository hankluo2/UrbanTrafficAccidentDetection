...
class args():
    ...
    # training args
    # epochs = 2000  # "number of training epochs, default is 2"
    ...
    channels = 3
    size = 256
    videos_dir = 'datasets/suzhou/train'
    time_steps = 5
...
def check_cuda():
    return bool(torch.cuda.is_available() and args.gpu is not None)
...
def train():
    ...
    optimizer_G = Adam(generator.parameters(), lr=args.g_lr_init)
    optimizer_D = Adam(discriminator.parameters(), lr=args.d_lr_init)
    ...
    trainloader = DataLoader(dataset=SequenceDataset(channels=args.channels,
                                                     size=args.size,
        ...
        for ep in range(args.epochs):
            ...
            for i, clips in enumerate(trainloader):
                ...
                for j, frames in enumerate(pbar):
                    ...
                    g_loss = int_loss + grad_loss + 2 * f_loss + 0.05 * adv_loss

                    d_loss = discriminator_loss(d_t, d_g)

                    # optimizer_D.zero_grad(retain_graph=True)
                    optimizer_D.zero_grad()  # modified
                    # d_loss.backward()
                    d_loss.backward(retain_graph=True)  # modified
                    optimizer_G.zero_grad()
                    # g_loss.backward()
                    g_loss.backward(retain_graph=True)  # modified
                    optimizer_D.step()
                    optimizer_G.step()
            ...
            if (ep + 1) % args.save_per_epoch == 0:
                ...
                torch.save(model_dict, os.path.join(args.save_model_dir,
                                                    f'suzhou_ckpt_{ep + 1}_{g_loss_mean}_{d_loss_mean}.pth'))
