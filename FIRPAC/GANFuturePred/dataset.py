...
class SequenceDataset(Dataset):
    def __init__(self, channels, size, videos_dir, time_steps):
        ...
        for f in sorted(os.listdir(videos_dir)):
            frames = glob.glob(os.path.join(videos_dir, f, '*.png'), recursive=True) + glob.glob(
                os.path.join(videos_dir, f, '*.jpg'), recursive=True)
            frames.sort()
...
class TestDataset(Dataset):
    def __init__(self, channels, size, videos_dir, time_steps):
        ...
        # self.videos = glob.glob(os.path.join(videos_dir, '*.png'), recursive=True)
        self.videos = glob.glob(os.path.join(videos_dir, '*'), recursive=True)
        self.videos.sort()
    ...
    def __getitem__(self, index):
        ...
        if self.channels == 1:
            frames = [
                cv2.imread(frame, cv2.IMREAD_GRAYSCALE).astype(np.float32)
                for frame in self.videos[index:index + self.time_steps]
            ]
        else:
            frames = [
                cv2.imread(frame, cv2.IMREAD_COLOR).astype(np.float32) for frame in self.videos[index:index + self.time_steps]
            ]
...
def base_transform(img, size):
    transform = A.Compose([A.Resize(height=size, width=size, always_apply=True, p=1.0)], p=1.0)
